...
from d1lod.d1lod.people import processing
...
class Interface:
    ...
    def insertModel(self):
        ...
        self.graph.insert_data(graph_name=self.graph.name,payload=sparql_data, blank_node=blank_node)
    ...
    def delete(self, s='?s', p='?p', o='?o'):
        ...
        return self.graph.delete_data(graph_name=self.graph.name, payload=payload_data, blank_node=blank_node)
    ...
    def findPersonURI(self, record):
        ...
        if 'last_name' in record and 'document' in record and self.model is not None:
            ...
            query = RDF.Statement(subject = RDF.Uri(self.graph.ns['d1dataset']+urllib.quote_plus(record['document'])),
