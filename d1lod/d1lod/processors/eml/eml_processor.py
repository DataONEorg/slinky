...
from ..util import element_text, get_doi, is_doi, is_orcid, get_orcid
...
class EMLProcessor(Processor):
    ...
    def process_spatial_coverage(self):
        ...
        # Make each bnode unique below by appending a number to each
        i = 0
        ...
        for geographic_coverage in self.scimeta.findall(
            ...
            place_bnode = RDF.Node(blank=f"spatialCoverage{i}")
            geo_bnode = RDF.Node(blank=f"geo{i}")
            additional_property_wkt_bnode = RDF.Node(blank=f"wkt{i}")
            additional_property_crs_bnode = RDF.Node(blank=f"crs{i}")
