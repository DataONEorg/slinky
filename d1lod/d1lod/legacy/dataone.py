...
def createSinceQueryURL(from_string, to_string, fields=None, start=0, page_size=1000):
    ...
    query_params += "+AND+dateModified:[" + from_string + "%20TO%20" + to_string + "]"

    rows = page_size
    start = start

    query_string = "%s?q=%s&rows=%s&start=%s&sort=dateModified+asc" % (
        base_url,
        query_params,
        rows,
        start,
...
def getIdentifiersSince(from_string, to_string, fields=None, page=1, page_size=1000):
    ...
    query_string = createSinceQueryURL(
        from_string, to_string, fields=fields, start=start
    )
...
def getDocumentIdentifiersSince(
    from_string, to_string=None, fields=None, page_size=1000
):
    ...
    query_string = createSinceQueryURL(
        from_string, to_string, fields=[], start=0, page_size=0
    )
...
def getSincePage(from_string, to_string, page=1, page_size=1000, fields=None):
    ...
    start = (page - 1) * page_size
    query_string = createSinceQueryURL(
        from_string, to_string, start=start, page_size=page_size
    )
    query_xml = util.getXML(query_string)
...
def getSystemMetadata(identifier, cache=False):
    ...
    query_string = "https://cn.dataone.org/cn/v1/meta/%s" % urllib.parse.quote_plus(
        identifier
    )
...
def getScientificMetadata(identifier, cache=False):
    ...
    query_string = "https://cn.dataone.org/cn/v1/object/%s" % urllib.parse.quote_plus(
        identifier
    )
...
def extractDocumentIdentifier(doc):
    ...
    if identifier is None:
        raise Exception(
            "Failed to add dataset because the identifier couldn't be processed."
        )
...
def getSolrIndexFields(identifier, fields=None):
    ...
    query_string = (
        "http://cn.dataone.org/cn/v1/query/solr/?fl="
        + ",".join(fields)
        + "&q=id:"
        + identifier_esc
        + "&rows=1&start=0"
