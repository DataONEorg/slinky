...
def update_graph():
    ...
    graph = Graph(
        host=VIRTUOSO_HOST, port=VIRTUOSO_PORT, name=VIRTUOSO_GRAPH, ns=NAMESPACES
    )
    ...
    for doc in docs:
        ...
        logging.info(
            "[%s] Queueing job add_dataset with identifier='%s'", JOB_NAME, identifier
        )
...
def add_dataset(graph, interface, identifier, doc=None):
    ...
    logging.info(
        "[%s] [%s] Adding dataset with identifier='%s'",
        JOB_NAME,
        identifier,
        identifier,
    )
    ...
    if doc is None:
        raise Exception(
            "No solr fields could be retrieved for dataset with PID %s.", identifier
        )
    ...
    logging.info(
        "[%s] [%s] Dataset added in: %f second(s).",
        JOB_NAME,
        identifier,
