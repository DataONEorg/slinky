...
def saveJSONFile(content, filename):
    ...
    with open(filename, "wb") as content_file:
        content_file.write(
            json.dumps(content, sort_keys=True, indent=2, separators=(",", ": "))
        )
...
def loadFormatsMap():
    ...
    r = requests.get(
        "https://raw.githubusercontent.com/ec-geolink/design/master/data/dataone/formats/formats.csv"
    )
...
def createIdentifierMap(path):
    ...
    if os.path.isfile(path):
        ...
        identifier_map = dict(
            list(zip(identifier_table.guid, identifier_table.filepath))
        )
...
def getIdentifierScheme(identifier):
    ...
    if (
        ...
        | identifier.startswith("http://doi.org/")
        | identifier.startswith("https://doi.org/")
        | identifier.startswith("http://dx.doi.org/")
        | identifier.startswith("https://dx.doi.org/")
    ):
        scheme = "doi"
...
def getIdentifierResolveURL(identifier):
    ...
    # Local
        ...
        return "https://cn.dataone.org/cn/v1/resolve/%s" % urllib.parse.quote_plus(
            identifier
        )
