...
from .people import *
import d1lod.legacy.util as util
import d1lod.legacy.dataone as dataone
import d1lod.legacy.validator as validator
...
class Interface:
    ...
    def add(self, s, p, o):
        ...
        if self.model is None:
            logging.info(
                "Failed to add triple to model because there was no current model."
            )
            return
    ...
    def addDatasetTriples(self, dataset_node, doc):
        ...
        if abstract_element is not None:
            self.add(
                dataset_node, "schema:description", RDF.Node(abstract_element.text)
            )
        ...
        if all(
            ele is not None
            for ele in [bound_north, bound_east, bound_south, bound_west]
        ):
            if (
                bound_north.text == bound_south.text
                and bound_west.text == bound_east.text
            ):
                wktliteral = "POINT (%s %s)" % (bound_north.text, bound_east.text)
            else:
                wktliteral = "POLYGON ((%s %s, %s %s, %s %s, %s, %s))" % (
                    bound_west.text,
                    bound_north.text,
                    bound_east.text,
                    bound_north.text,
            ...
            self.add(
                dataset_node, "schema:hasGeometryAsWktLiteral", RDF.Node(wktliteral)
            )
        ...
        if obsoletes_node is not None:
            ...
            self.add(
                dataset_node,
                "prov:wasRevisionOf",
                RDF.Uri(self.graph.ns["d1dataset"] + other_document_esc),
            )

        # Landing page
        self.add(
            dataset_node,
            "schema:hasLandingPage",
            RDF.Uri("https://search.dataone.org/#view/" + identifier_esc),
        )
        ...
        if len(resource_map_identifiers) > 0:
            for resource_map_node in resource_map_identifiers:
                ...
                digital_objects = dataone.getAggregatedIdentifiers(
                    resource_map_identifier
                )
    ...
    def addDigitalObjectTriples(self, dataset_identifier, digital_object_identifier):
        ...
        if data_meta is None:
            raise Exception(
                "System metadata for data object %s was not found. Continuing to next data object."
        ...
        if checksum_node is not None:
            ...
            self.add(
                do_node,
                "schema:hasChecksumAlgorithm",
                RDF.Node(checksum_node.get("algorithm")),
            )
        else:
            raise Exception(
                "Sysmeta XML for PID %s had no checksum element"
                % digital_object_identifier
            )
        ...
        if size_node is not None:
            ...
            raise Exception(
                "Sysmeta XML for PID %s had no size element" % digital_object_identifier
            )
        ...
        if format_id_node is not None:
            if format_id_node.text in self.formats:
                self.add(
                    do_node,
                    "schema:hasFormat",
                    RDF.Uri(self.formats[format_id_node.text]["uri"]),
                )
            else:
                raise Exception(
                    "Format %s not found in list of known formats."
                    % format_id_node.text
                )
        else:
            raise Exception(
                "Sysmeta XML for PID %s had no formatId element"
                % digital_object_identifier
            )
        ...
        if date_uploaded_node is not None:
            ...
            raise Exception(
                "Sysmeta XML for PID %s had no dataUploaded element"
                % digital_object_identifier
            )
        ...
        if authoritative_mn is not None:
            self.add(
                do_node,
                "schema:hasAuthoritativeDigitalGraph",
                "d1node:" + authoritative_mn.text,
            )
        else:
            raise Exception(
                "Sysmeta XML for PID %s had no authoritativeMemberNode element"
                % digital_object_identifier
            )
        ...
        if replica_mns is None:
            raise Exception(
                "Sysmeta XML for PID %s had no replica element"
                % digital_object_identifier
            )
        ...
        for replica_mn in replica_mns:
            ...
            if replica_node is not None:
                self.add(
                    do_node,
                    "schema:hasReplicaDigitalGraph",
                    "d1node:" + replica_node.text,
                )
            else:
                raise Exception(
                    "Sysmeta XML for PID %s had no replicaMemberNode element"
                    % digital_object_identifier
                )
        ...
        if origin_mn is not None:
            self.add(
                do_node, "schema:hasOriginDigitalgraph", "d1node:" + origin_mn.text
            )
        else:
            raise Exception(
                "Sysmeta XML for PID %s had no originMemberNode element"
                % digital_object_identifier
            )
    ...
    def addPersonTriples(self, uri, record):
        ...
        logging.info(
            "Adding person triples for person with uri '%s' and record '%s'",
            uri,
            record,
        )
        ...
        if "organization" in record:
            ...
            if self.organizationExists(organization_name):
                ...
                logging.info(
                    "Minted new organization URI of '%s' and adding triples.",
                    organization_uri,
                )
        ...
        if "role" in record and "document" in record:
            if record["role"] == "creator":
                self.add(
                    uri,
                    "schema:isCreatorOf",
                    "d1dataset:" + urllib.parse.unquote(record["document"]),
                )
    ...
    def addOrganization(self, record):
        ...
        if organization_uri is None:
            organization_uri = self.mintOrganizationPrefixedURIString()
            logging.info(
                "Organization was not found. Minted URI of %s", organization_uri
            )
    ...
    def addOrganizationTriples(self, uri, record):
        ...
        logging.info(
            "Adding organization triples for organization with uri '%s' and record '%s'",
            uri,
            record,
        )
    ...
    def findPersonURI(self, record):
        ...
        if "last_name" in record and "document" in record and self.model is not None:
            logging.info(
                "Attempting to match %s via last name and wasRevisionOf.", record
            )

            query = RDF.Statement(
                subject=RDF.Uri(
                    self.graph.ns["d1dataset"]
                    + urllib.parse.unquote(record["document"])
                ),
            ...
            revised_document = RDF.Uri(
                self.graph.ns["d1dataset"] + revised_documents[0]
            )
            ...
            if len(result) == 1 and "person" in result[0]:
                ...
                person_uuid_search = re.search(
                    r"<%s(.*)>" % self.graph.ns["d1person"], result_string
                )
    ...
    def findOrganizationURI(self, record):
        ...
        if "name" in record:
            ...
            logging.info(
                "Found organiztion match for organization URI %s.", organization_uri
            )
    ...
    def addIdentifierTriples(self, node, identifier):
        ...
        if resolve_url is not None:
            self.add(
                identifier_node, "schema:hasIdentifierResolveURL", RDF.Uri(resolve_url)
            )
        ...
        if scheme != "local-resource-identifier-scheme":
            dataone_resolve_url = (
                "https://cn.dataone.org/cn/v1/resolve/%s"
                % urllib.parse.unquote(identifier)
            )
            self.add(
                identifier_node,
                "schema:hasIdentifierResolveURL",
