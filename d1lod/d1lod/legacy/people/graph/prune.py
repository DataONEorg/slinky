...
def prune(filename, type):
    ...
    # Do the processing
    if type == "people":
        prunePeople(input_file, retained_file, rejected_file, people_field_names)
    else:
        pruneOrganizations(
            input_file, retained_file, rejected_file, organization_field_names
        )
...
def prunePeople(input_file, retained_file, rejected_file, field_names):
    ...
    input_reader = unicodecsv.DictReader(input_file, field_names)

    retained_writer = unicodecsv.DictWriter(retained_file, field_names)

    rejected_writer = unicodecsv.DictWriter(rejected_file, field_names)

    # Write headers
    retained_writer.writerow(dict(list(zip(field_names, field_names))))
    rejected_writer.writerow(dict(list(zip(field_names, field_names))))
    ...
    patterns = {
        "wg": re.compile(r"NCEAS:?\s*\d+"),
        "justnumbers": re.compile(r"^\d*$"),
        "junknames": re.compile(r"^[a-z]{3,4}\s*\d*$"),
        "noletters": re.compile(r"^[^a-zA-Z\\u0000-\\u007F]+$"),
        "journal article": re.compile(r"\d+:\d+-\d+"),
    }
    ...
    for row in input_reader:
        ...
        if re.compile(r"Select state or territory here").search(row["address"]):
            row["address"] = ""
        ...
        for pattern in patterns:
            if patterns[pattern].search(row["organization"]):
                ...
                try:
                    bytes(row["organization"].encode("utf-8"))
                except UnicodeEncodeError:
                    prune_organization = False
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(
            "Incorrect number of arguments. Please specifiy filename and " "filetype."
        )
