...
def uniquify(filename, type):
    ...
    # Do the processing
    if type == "people":
        uniquifyPeople(input_file, output_file, people_field_names)
    else:
        uniquifyOrganizations(input_file, output_file, organization_field_names)
...
def uniquifyPeople(input_file, output_file, field_names):
    ...
    output_writer = unicodecsv.DictWriter(output_file, field_names)

    # Write headers
    output_writer.writerow(dict(list(zip(field_names, field_names))))
    ...
    with open("people_unique.json", "wb") as f:
        f.write(json.dumps(seen, sort_keys=True, indent=2, separators=(",", ": ")))
