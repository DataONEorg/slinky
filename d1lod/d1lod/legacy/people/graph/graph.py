    ...
    for key in jsonfile:
        ...
        for record in records:
            ...
            if len(record["organization"]) > 0:
                existing_uri = findOrganizationUri(
                    organizations, record["organization"]
                )

                if existing_uri is None:
                    existing_uri = mintOrganizationUri(
                        organizations, record["organization"], ns
                    )
        ...
        for salutation in salutations:
            addStatement(model, identifier_uri, ns["glview"] + "namePrefix", salutation)

        # Full name => glview:nameFull
        for full_name in full_names:
            addStatement(model, identifier_uri, ns["glview"] + "nameFull", full_name)

        # First name => glview:nameGiven
        for given_name in first_names:
            addStatement(model, identifier_uri, ns["glview"] + "nameGiven", given_name)

        # Last name => glview:nameFamily
        for last_name in last_names:
            addStatement(model, identifier_uri, ns["glview"] + "nameFamily", last_name)
        # Organizations
        ...
        for email in email_addresses:
            addStatement(
                model, identifier_uri, ns["foaf"] + "mbox", RDF.Uri("mailto:" + email)
            )

        # Address
        for address in mailing_addresses:
            addStatement(model, identifier_uri, ns["glview"] + "address", address)
...
def createOrganizationGraph(filename, ns={}):
    ...
    if os.path.isfile("organization_uris.csv"):
        ...
        organizations = pandas.Series.from_csv(
            "organization_uris.csv", encoding="utf-8"
        ).to_dict()
    ...
    for key in jsonfile:
        ...
        # Full name => glview:namePrefix
        for name in names:
            addStatement(model, identifier_uri, ns["rdf"] + "label", name)
...
def mintPeopleUri(people, key, ns):
    ...
    print(
        "Minting new person URI for %s <=> %s."
        % (key.encode("utf-8"), identifier_uri_string.encode("utf-8"))
    )
...
def mintOrganizationUri(organizations, key, ns):
    ...
    print(
        "Minting new organization URI for %s <=> %s."
        % (key.encode("utf-8"), identifier_uri_string.encode("utf-8"))
    )
