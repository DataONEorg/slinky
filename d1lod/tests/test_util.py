...
def test_can_match_doi_strings():
    ...
    assert (
        util.getIdentifierScheme("somethingsomething")
...
def test_can_determine_resolve_urls():
    # DOIs
    assert (
        util.getIdentifierResolveURL("http://doi.org/10.asdf")
        == "http://doi.org/10.asdf"
    )
    assert util.getIdentifierResolveURL("doi:/10.asdf") == "http://doi.org/10.asdf"
    assert (
        util.getIdentifierResolveURL("http://dx.doi.org/10.asdf")
        == "http://doi.org/10.asdf"
    )
    ...
    # DataOne PIDs
    assert (
        util.getIdentifierResolveURL("asdf")
        == "https://cn.dataone.org/cn/v1/resolve/asdf"
    )
