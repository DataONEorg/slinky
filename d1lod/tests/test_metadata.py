...
from d1lod.d1lod.metadata import *
...
def test_iso_ignores_metadata_contacts():
    ...
    doc = ET.parse('./data/metadata/iso/contact.xml')
...
def test_iso_ignores_empty_elements():
    ...
    doc = ET.parse('./data/metadata/iso/emptystrings.xml')
    assert iso.process(doc) == []


def test_can_process_a_simple_file():
    doc = ET.parse('./data/metadata/iso/creator.xml')
    assert iso.process(doc) == [
        {
...
def test_removes_the_email_when_its_invalid():
    doc = ET.parse('./data/metadata/iso/creator_bad_email.xml')
    assert iso.process(doc) == [
        {
            'address': 'delivery point line 1 delivery point line 2 city state zip country',
            'name': 'org_name',
...
def test_iso_can_process_a_ncei_file():
    ...
    doc = ET.parse('./data/metadata/iso/ncei.xml')
    records = iso.process(doc)
